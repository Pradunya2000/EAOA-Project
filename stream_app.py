...
# --- 5. Memory Manager (New - from your memory_manager.py) ---
...
def save_to_memory(goal: str, task: str, result: str,
    ...
    # Simple check to avoid duplicates for the same task text.
    # Note: A more robust check might involve comparing metadata, but this is a good start.
...
def clear_memory():
    try:
        client = PersistentClient(path="./chroma_memory")
        client.delete_collection("memory")
        del client
        gc.collect()
        time.sleep(1)
        shutil.rmtree("./chroma_memory", ignore_errors=True)
        # st.success("✅ Memory cleared successfully")
        Chroma(persist_directory="./chroma_memory", embedding_function=embeddings).persist()
    except Exception as e:
